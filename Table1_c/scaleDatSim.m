...
savingfoldername = ['.\SimResults\',num2str(N)];
...
for iter = 1:Ncases  % changing TBinGen, to estimate mean, std, etc.
    ...
    settings.Y = Yo;
    F0(iter) = sum(TBin(:)); %conventional (None)
    ...
    Fb(iter) = sum(TBposBld(:).*TBin(:)); % blind (Randomnized Squences)
    ...
    Fgen(iter) = sum(TBposGE(:).*TBin(:)); % general (GAS)
...
%%
timestp = datestr(now,'yyyymmdd_HH_MM_SS');
savefileName = sprintf('Ncases%d_N%d_Mp%d_M%d_',Ncases,N,Mp,M);
save([savingfoldername,'\',savefileName,timestp,'.mat'],'Results','settings');
